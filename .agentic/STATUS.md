...
- [x] TASK-025 — v1.13.0 release bookkeeping: health-report twins, three new task templates (`VERDICT.md`/`CHORE_MAINTENANCE.md`/`SPIKE.md`), three new context modules (`data-integrity`/`api-design-patterns`/`error-handling`), three new eval scenarios, bash 3.2 health-report fix, CI (Full) macOS budget raise, `protocol_version` sweep to 1.13.0, annotated tag + release published (see `.agentic/tasks/TASK-025-v1130-release-bookkeeping.md`)
...
- [x] TASK-033 — v1.14.0 release bookkeeping: VERSION→1.14.0, protocol_version sweep (28 files), 12 eval artifacts regenerated, CHANGELOG `[1.14.0] - 2026-09-10`, ROADMAP refresh; tag/release published with 3 assets (see `.agentic/tasks/TASK-033-v1140-release-bookkeeping.md`)
- [x] TASK-034 — CI retry hardening: bounded retry (3 attempts, 2s exponential backoff) on all 5 `Install-CiModule` call sites in ci.yml / ci-full.yml to absorb transient PSGallery 403s (see `.agentic/tasks/TASK-034-ci-retry-hardening.md`)
- [x] TASK-035 — Module eval scenarios for 5 uncovered context modules: `performance-hot-path`, `accessibility-contrast`, `i18n-string-extraction`, `mobile-responsive-breakpoint`, `testing-ci-config`; corpus 12→17, both eval twins 17/17 (see `.agentic/tasks/TASK-035-module-eval-scenarios.md`)
...
- [x] TASK-041 — verify.ps1 bash stdin redirect hardening: `$stdinNull` gate in `Invoke-Check` feeds `$null` stdin to any Windows bash-routed check (extensionless `#!/usr/bin/env bash` launchers and direct `bash <script.sh>` checks incl. `handoff-gate` and `evals-sh`), so MSYS/Git Bash checks under a non-interactive parent see EOF instead of blocking; two Pester regression tests cover EOF and exit-code propagation, and the full `Verify.Tests.ps1` suite passes 38/38 (see `.agentic/tasks/TASK-041-verify-ps1-bash-stdin-redirect.md`)
- [x] TASK-042 — Agent Skills frontmatter interop: all six registry SKILL.md files now carry the agentskills.io-compatible YAML frontmatter (`name`, `description`, `metadata`) above the unchanged protocol sections; `validate-skills.sh`/`.ps1` validate frontmatter when present and tolerate its absence for adopter skills; five new Bats + five new Pester regression tests; five-case matrix (valid/name-mismatch/empty-desc/unclosed/legacy) yields identical exit codes in both twins (see `.agentic/tasks/TASK-042-skills-format-alignment.md`)
