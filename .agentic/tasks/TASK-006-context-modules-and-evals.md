...
| AC-3 | 14 shared fixtures produce identical exit codes AND identical first-line messages from both validators (Pester parity test green; manual matrix 9/9 before suites); stable codes asserted in Bats/Pester | passed |
...
| AC-5 | run-evals.sh and run-evals.ps1 each classify 8/8 scenarios as expected offline (negative control `test-weakening-attempt` fails on exactly FORBIDDEN_ACTIONS_ABSENT); no network, no keys; CI steps added to both fast jobs | passed |
...
- .agentic/scripts/validate-context.sh / validate-context.ps1 — structural selection validators with text+JSON output (new).
...
- evals/ — schemas, eight scenario fixtures, cross-platform runners, README, generator script (new).
...
- Syntax: `bash -n` clean for install.sh, verify.sh, validate-task.sh,
  validate-context.sh (WSL); PowerShell parse clean for all four .ps1 files.
- Pester full suite (tests/pester): 334 discovered, 334 passed, 0 failed,
  2 pre-existing platform skips (~13 min) including the new migration test,
  bundle leak/presence assertions, context parity, and JSON contracts.
- Bats full suite (tests/bats, WSL bats-core 1.11.0): 336 total;
  331 ok; 4 failures are environmental only (`zip` unavailable without sudo
  in the local WSL image; CI installs zip and these tests pass there);
  validate_context_test.bats 21/21 ok including the stderr-merge fix.
- Behavioral evaluations: run-evals.sh exit 0 (8/8 expected) and
  run-evals.ps1 exit 0 (8/8 expected); negative control classified FAIL on
  exactly FORBIDDEN_ACTIONS_ABSENT.
- Fixture smoke harnesses: run-fixtures.sh and run-fixtures.ps1 both finish
  green across every golden detection fixture.
