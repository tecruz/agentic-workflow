...
MANAGED_FILES=(
    ...
    ".agentic/templates/CHORE_MAINTENANCE.md"
    ".agentic/templates/SPIKE.md"
    ".agentic/templates/VERDICT.md"
    ...
    ".agentic/context/performance/MODULE.md"
    ".agentic/context/accessibility/MODULE.md"
    ".agentic/context/i18n/MODULE.md"
    ".agentic/context/mobile-adaptive/MODULE.md"
    ".agentic/context/testing-infrastructure/MODULE.md"
    ...
    ".agentic/skills/release-verification/SKILL.md"
    ".agentic/skills/dependency-audit/SKILL.md"
    ".agentic/skills/migration-rollback/SKILL.md"
    ".agentic/skills/perf-investigation/SKILL.md"
    ".agentic/scripts/validate-context.sh"
    ".agentic/scripts/validate-context.ps1"
    ".agentic/scripts/validate-skills.sh"
    ".agentic/scripts/validate-skills.ps1"
    ".agentic/scripts/validate-handoff.sh"
    ".agentic/scripts/validate-handoff.ps1"
    ".agentic/scripts/health-report.sh"
    ".agentic/scripts/health-report.ps1"
...
safe_remove_file() {
    ...
    [ -e "$dst" ] || return 0
    if ! rm -f "$dst"; then
        echo "ERROR: failed to remove '$rel'." >&2
        return 1
    fi
    [ ! -e "$dst" ] || return 1
}
...
write_generated_candidate() {
    ...
    if [ -n "$detection" ]; then
        if ! {
            ...
            rm -f "$tmp" 2>/dev/null || true
            echo "ERROR: failed to write generated candidate temporary file." >&2
            return 1
        fi
        ...
        if ! mv "$tmp" "$TARGET_DIR/$gen_rel"; then
            rm -f "$tmp" 2>/dev/null || true
            echo "ERROR: failed to promote generated candidate." >&2
            return 1
        fi
        echo "Candidate contract written to $gen_rel"
    else
        rm -f "$tmp" 2>/dev/null || true
        if ! safe_remove_file "$gen_rel"; then
            echo "ERROR: failed to remove stale generated candidate." >&2
            return 1
        fi
